...
@section('content')
    <h1><i class="fas fa-comment-dots"></i> &nbsp;Companies Complains</h1>
            ...
            @if($con->count())
                @foreach($con as $c)
                    <tr>
                        ...
                        <td><a class="btn btn-primary btn-sm btn-block mb-2" href="{{route('admin.companycontact.show',[$c->id])}}">View</a>
    ...
    <!-- Modal -->
    <form action="{{route('admin.companycontact.destroy', [$c->id])}}" method="POST">
    @csrf
        @method('DELETE')
